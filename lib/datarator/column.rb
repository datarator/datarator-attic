...
module Datarator
  class Column
    ...
    def nested=(nested)
      raise ArgumentError, "nested columns expected, but not found for: #{name}" if Types.nested?(self) && nested.nil?
      raise ArgumentError, "nested columns not expected, but found for: #{name}" if !Types.nested?(self) && !nested.nil?

      unless nested.nil?
        raise ArgumentError, "nested columns should be nil or array, but was: #{nested.inspect}" unless nested.is_a?(Array)
    ...
    def ==(other)
      # TODO: all other vars
      name == other.name && type == other.type && empty_percent == other.empty_percent && options == other.options && nested == other.nested
    end
