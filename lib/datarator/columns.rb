  ...
  class Columns
    ...
    class << self
      def from_json(out_context, data)
        raise ArgumentError, 'expected parsed json data having columns element' if data.nil? || data['columns'].nil?
    ...
    def column_by_name(name)
      ...
      raise ArgumentError, "no column found for name: #{name}" if column.nil?
      column
    end

    def column_by_name=(column)
      raise ArgumentError, 'column has to be of type Column' unless column.is_a? Column
      raise ArgumentError, 'column name has to be unique' if @column_by_name.key? column.name
    ...
    def columns=(columns)
      ...
      raise ArgumentError, "columns should be nil or array, but was: #{columns.inspect}" unless columns.is_a?(Array)
      columns.each do |column|
